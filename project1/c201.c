...
void List_Dispose( List *list ) {
	ListElementPtr first = list->firstElement; 
	while(first != NULL){ //dokud neni seznam prazdny, projizdime ho
		ListElementPtr dispose = first->nextElement; //nastavime druhy prvek do promenne dispose
		free(first); //uvolnime

		first = dispose; //prvek v dispose dame do first
	}
	list->activeElement = NULL;
	list->firstElement = NULL;
}
...
void List_InsertFirst( List *list, int data ) {
	ListElementPtr prvek = malloc(sizeof(int) + sizeof(struct ListElement)); //nalocovani pameti
...
void List_DeleteFirst( List *list ) {
	...
	if(first == list->activeElement){
			list->activeElement = NULL;
	}
	list->firstElement = first->nextElement;
	free(first); //uvolneni first
}
...
void List_DeleteAfter( List *list ) {
	...
	ListElementPtr next = list->activeElement->nextElement;
	list->activeElement->nextElement = next->nextElement;
	free(next); //uvolneni pameti
}
...
void List_InsertAfter( List *list, int data ) {
	...
	prvek->data = data;
	prvek->nextElement = list->activeElement->nextElement;
	list->activeElement->nextElement = prvek;
} 
