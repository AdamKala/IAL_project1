...
void Queue_Init( Queue *queue ) {
	if(queue == NULL){
		...
		for(int i = 0; i < QUEUE_SIZE; i++){ //projizdime
			queue->array[i] = '*'; // nastavujeme hodnoty na *
		}
		queue->firstIndex = 0; //nastavujeme indexy
		queue->freeIndex = 0;
	}
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	if(Queue_IsEmpty(queue)){ //pokud fronta prazdna - error
...
void Queue_Enqueue( Queue *queue, char data ) {
	if(Queue_IsFull(queue)){ //pokud fronta prazdna - error
